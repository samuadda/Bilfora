...
export default function DashboardRecentActivity({
	...
	return (
		<div className="bg-surface rounded-3xl border border-border shadow-sm overflow-hidden flex flex-col h-full">
			<div className="p-5 sm:p-6 border-b border-border flex items-center justify-between flex-shrink-0">
				<div>
					<h3 className="text-lg font-bold text-foreground">النشاطات الأخيرة</h3>
					<p className="text-xs text-subtle mt-1">آخر التحديثات على حسابك</p>
				</div>
			</div>
			...
			{activities.length > 0 ? (
					...
					{activities.map((act, i) => {
						...
						return (
							<m.div
								...
								className={cn(
									"p-5 hover:bg-surface-2 transition-colors group",
							...
							>
								<div className="flex items-start gap-4">
									...
									<div className="flex-1 min-w-0">
										<h4 className="text-foreground font-bold text-sm mb-1 truncate">
											{act.title}
										</h4>
										<p className="text-xs text-subtle flex items-center gap-2 flex-wrap">
									...
									{act.invoiceId && (
										<ArrowLeft
											size={16}
											className="text-disabled group-hover:text-brand transition-colors flex-shrink-0 mt-1"
										/>
									)}
								</div>
							</m.div>
				...
				<div className="flex-1 flex items-center justify-center p-12 text-center">
					<div className="w-16 h-16 bg-surface-2 rounded-full flex items-center justify-center mx-auto mb-4">
						<FileText className="text-disabled w-8 h-8" />
					</div>
